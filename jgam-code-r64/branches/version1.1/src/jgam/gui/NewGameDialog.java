...
public class NewGameDialog extends JDialog {
    ...
    private void jbInit() throws Exception {
        ...
        String loc = "Local";   // on linux this does not work :(
        try {
           loc = InetAddress.getLocalHost().getCanonicalHostName();
        } catch(Exception ex) {
           if (Boolean.getBoolean("jgam.debug")) {
             System.err.println("Exception while getting the local host name");
             ex.printStackTrace();
           }
        }
