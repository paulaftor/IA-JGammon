...
public class FeatureTrainer extends JFrame {
    ...
    private void rolloutSamples() throws Exception {
        ...
        ai.init();
        ((NormalNeuralNetAI) ai.getAIForCategory(CombiAI.Mode.NORMAL)).setNeuralNet(state.neuralNet);
        ...
        while (!state.boardSet.isEmpty() && !interrupted) {
            ...
            // log
            log.write(BoardSnapshot.encodeBase64(board)+" "+target+"\n");
        }
    ...
    private double rollout(TrainingSetup trainingSetup, boolean record) throws CannotDecideException {
        ...
        while (trainingSetup.winstatus() == 0
            ...
            MoveChain moves = ai.makeMoves(trainingSetup);

            for (int i = 0; i < moves.size(); i++) {
                trainingSetup.performMove(moves.get(i));
                /*                trainingSetup.debugOut();
        ...
        if (trainingSetup.winstatus() == 1) {
            ...
            Mode cat = CombiAI.getCategory(trainingSetup, trainingSetup.getPlayerAtMove());
            try {
                return ((EvaluatingAI) ai.getAIForCategory(cat)).probabilityToWin(trainingSetup);
            }catch(CannotDecideException ex) {
                return ((EvaluatingAI) ai.getAIForCategory(CombiAI.Mode.NORMAL)).probabilityToWin(trainingSetup);
            }
        }
