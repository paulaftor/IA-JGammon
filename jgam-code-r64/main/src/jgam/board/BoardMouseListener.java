...
public class BoardMouseListener implements MouseListener, MouseMotionListener,
    ...
    private Set<MoveChain> possibleMoves;
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        SingleMove m = player.getDoubleClickMove(point);
    ...
    public void mouseReleased(MouseEvent e) {
        ...
        if (possibleMoves != null) {
            for (MoveChain chain : possibleMoves) {
                assert chain.isConsecutive();
                if (chain.lastTo() == endPoint) {
                    for (SingleMove singleMove : chain) {
                        player.handleMove(singleMove);
                    }
                    return;
                }
            }
            ...
            if (jgam.getFrame().showPreviewCheckers()) {
                for (MoveChain chain : possibleMoves) {
                    ...
                    if (chain.lastTo() == 0) {
                        ...
                        int point = board.playerToNativePoint(chain.lastTo(),
                                    draggingPlayer);
                        board.paintChip( -draggingPlayer, g, point,
                                oldSetup.getPoint(chain.player(), chain.lastTo()));
                    }
                }
            }
