...
public abstract class EvaluatingAI implements AI {
    ...
    public static final double DOUBLE_THRESHOLD = .25;
    ...
    public MoveChain makeMoves(BoardSetup boardSetup)
        ...
        for (Iterator iter = list.iterator(); iter.hasNext(); index++) {
            ...
            double value = probabilityToWin(setup);
            if (value > bestValue) {
                bestValue = value;
                bestIndex = index;
            }
     ...
     * 
     * The result is an approximation of the probability to win.
     * 
     * @param setup BoardSetup
     * @return double between 0 and 1.
     */
    public abstract double probabilityToWin(BoardSetup setup)
            throws CannotDecideException;
    ...
    public int rollOrDouble(BoardSetup boardSetup) throws CannotDecideException {
        if (boardSetup.mayDouble(boardSetup.getPlayerAtMove())) {
            double eval = probabilityToWin(boardSetup);
