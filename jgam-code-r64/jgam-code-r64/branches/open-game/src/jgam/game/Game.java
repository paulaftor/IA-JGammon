...
public class Game {
    ...
    private final PrintWriter diceLogger;
    ...
    private final BlockingQueue messageQueue = new BlockingQueue();
    ...
    private void run() {
        ...
        try {
            ...
            informPlayers(new PlayerMessage(PlayerMessage.MessageType.START_GAME));
            ...
            ex.printStackTrace();
            informPlayers(new PlayerMessage(null, PlayerMessage.MessageType.ABNORMAL_ABORT, ex));
            messageQueue.abort();
            jgam.clearGame();
        } finally {
            player1.dispose();
    ...
    private void atWinning() {
        ...
        PlayerMessage pm = new PlayerMessage(winner, PlayerMessage.MessageType.GAME_OVER);
        pm.setValue(wintype);
    ...
    private void atMove() throws InterruptedException, GameException {
        ...
        case MOVE:
            ...
            SingleMove move = (SingleMove) pm.getObject();
        ...
        case YIELD:
            if (pm.getOwner() != getCurrentPlayer()) {
                throw new IllegalStateException("Only the current player may 'yield': " + pm);
            }
            setState(STATE_AT_DECISION);
            switchPlayers();
            informPlayers(pm);
...
//            Player loser = getOtherPlayer(winner);
    ...
    private void atDecision() throws InterruptedException, GameException {
        ...
        informPlayers(new PlayerMessage(currentPlayer, PlayerMessage.MessageType.MY_TURN));
...
//            Player loser = getOtherPlayer(winner);
    ...
    private PlayerMessage receiveMessage() throws InterruptedException, GameException {
        ...
        if (pm.getMessage() == PlayerMessage.MessageType.ABNORMAL_ABORT) {
    ...
    private void chooseBeginner() throws InterruptedException, GameException {
        ...
        do {
            diceRoller.rollDices(this);
            PlayerMessage pm = receiveMessage();
            if (pm.getMessage() != PlayerMessage.MessageType.DICES) {
