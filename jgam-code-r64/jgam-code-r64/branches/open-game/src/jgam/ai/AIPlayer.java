...
//            }
    ...
    private void answerDouble() throws CannotDecideException {
        if (ai.takeOrDrop(game.getGameBoard()) == AI.TAKE) {
            game.sendPlayerMessage(new PlayerMessage(AIPlayer.this, PlayerMessage.MessageType.DOUBLE_TAKEN));
        } else {
            game.sendPlayerMessage(new PlayerMessage(AIPlayer.this, PlayerMessage.MessageType.DOUBLE_DROPPED));
        }
    }
    ...
    private void myturn() throws CannotDecideException {
        if (game.getGameBoard().mayDouble(getNumber())) {
            ...
            if (rollOrDouble == AI.DOUBLE) {
                game.sendPlayerMessage(new PlayerMessage(AIPlayer.this, PlayerMessage.MessageType.DOUBLE));
                return;
            }
        }
        game.sendPlayerMessage(new PlayerMessage(AIPlayer.this, PlayerMessage.MessageType.ROLL));
    }
