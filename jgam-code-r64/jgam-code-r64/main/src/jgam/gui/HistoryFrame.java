...
public class HistoryFrame extends JFrame {
    ...
    private List<SingleMove> movesToOverlay = null;
    ...
    private void jbInit() throws Exception {
        ...
        jTabbedPane.add(board, msg.getString("board")); //$NON-NLS-1$
        {
            ...
            statTableModel.setValueAt("maydouble", 2, 0);
            statTableModel.setValueAt("checkers", 3, 0);
            statTableModel.setValueAt("piptogo", 4, 0);
            statTableModel.setValueAt("pipsofar", 5, 0);
            statTableModel.setValueAt("doubles", 6, 0);
            statTableModel.setValueAt("hits", 7, 0);
            statTableModel.setValueAt("ai_eval", 8, 0);
     ...
     * make this visible!
     */
    public static void show(History history) {
        HistoryFrame frame = new HistoryFrame(history);
        frame.pack();
    ...
    private void updateStatistics(int index) {
        ...
        statTableModel.setValueAt("farbe1", 1, 1);
        statTableModel.setValueAt("farbe2", 1, 2);
        statTableModel.setValueAt(gameStatistics.mayDouble(1) ? "true"
                : "false", 2, 1);
        statTableModel.setValueAt(gameStatistics.mayDouble(2) ? "true"
                : "false", 2, 2);
    ...
    private String makeAIString(BoardSetup setup, int player) {
        return numberFormatter.format(gameStatistics.getAIEval(setup, player))
                ...
                + numberFormatter.format(gameStatistics.getAIMoves(setup,
                    player)) + "] ("
                + gameStatistics.getAICategory(setup, player) + ")";
    }
    ...
    private class MovesPainter implements Paintable {
        ...
        public void paint(Graphics g) {
            ...
            for (SingleMove move : movesToOverlay) {
