...
public class JGammon implements ActionListener {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            for (cur = 0; cur < args.length; cur++) {
                ...
                } else if (args[cur].startsWith("-mat")) {
                    theJGammon = new JGammon();
                    System.err.println("-AUTO* ONLY FOR DEBUG!!");
                    Player ai = new AIPlayer(new NeuralCombiAI());
                    JGammonConnection gc;
                    if (args[cur].equals("-mats")) {
                        gc = new JGammonConnection(1777, "AutoServer", null,
                                null);
                    } else {
                        gc = new JGammonConnection("localhost", 1777,
                                "AutoClient");
                    }
                    Player remote = new JGammonNetPlayer(gc);
                    if (args[cur].equals("-mats")) {
                        theJGammon.game = new Game(gc, ai, remote, theJGammon);
                    } else {
                        theJGammon.game = new Game(gc, remote, ai, theJGammon);
                    }
                    theJGammon.game.start();
                    theJGammon.getFrame().getBoard().useBoardSetup(
                        theJGammon.game.getGameBoard());
                    return;
     ...
     */
    private static ArrayList<BoardImplementation> boardImplementations = 
            new ArrayList<BoardImplementation>();
    static {
        try {
    ...
    private static void initBoardImplementations() throws Exception {
        Enumeration<URL> urls = JGammon.getExtClassLoader().getResources(
            "META-INF/services/jgam.board.BoardImplementation");
        while (urls.hasMoreElements()) {
            ...
            while (is.ready()) {
                ...
                try {
                    try {
                        Object newInstance = Class.forName(line).newInstance();
                        BoardImplementation impl = BoardImplementation.class.cast(newInstance);
                        boardImplementations.add(impl);
                    } catch (ClassNotFoundException ex) {
                        boardImplementations.add(new ResourceBoardImplementation(line));
                    }
                } catch (Exception ex) {
                    System.err
                    .println("Error while instantiating board implementation for: "
                            + line);
                    ex.printStackTrace();
                }
            }
     ...
     */
    public static List<BoardImplementation> getBoardImplementations() {
        ...
        if (extClassLoader == null) {
            try {
                ...
                if (extDir.isDirectory()) {
                    ...
                    // fixed 2012: would throw NPE if "ext" did not exist
