 ...
 * @version 1.0
 */
public abstract class SimpleNeuralNet implements NeuralNet, Serializable {

    /**
     * serial number for aving and loading.
     * Must be adapted if data structures change
     */
    private static final long serialVersionUID = -877949076297527230L;
    ...
    public synchronized double[] applyTo(double[] data, double[] target) {
        ...
        for (int j = 0; j < hiddensize; j++) {
            ...
            hiddenRegister[j] = activate(sum);
        }

        for (int k = 0; k < outsize; k++) {
            // bias
            sum = outweights[k][hiddensize];
     ...
     * @return double
     */
    protected abstract double activate(double val);

    /**
     * derivative of the activation function.
     * Caution, the argument is act(x) not x.
     *
     * given: act(x)
     * return: act'(x)
     *
     * @param activated act(x)
     * @return act'(x)
     */
    protected abstract double activateDiff(double activated);
    ...
    public synchronized double error(double[] data, double[] target) {
        double[] is = applyTo(data, outRegister);
        double mse = 0;
        for (int i = 0; i < is.length; i++) {
            double delta = is[i] - target[i];
            mse +=  delta * delta;
        }
        return mse;
    }
    ...
    public synchronized void train(double[] data, double[] target) {
        ...
        for (int i = 0; i < outsize; i++) {
            // double diff = outRegister[i] * (1 - outRegister[i]);
            double diff = activateDiff(outRegister[i]);
            outError[i] = (target[i] - outRegister[i]) * diff;
        }
        ...
        for (int i = 0; i < hiddensize; i++) {
            // that is the derivative of the activation function
            // hiddenError *= activate'(activate(weighted sum))
            // hiddenError[i] *= hiddenRegister[i] * (1 - hiddenRegister[i]);
            hiddenError[i] *= activateDiff(hiddenRegister[i]);
        }
    ...
    public void dump() {
        for (int i = 0; i < hiddensize; i++) {
            ...
            for (int j = 0; j < insize; j++) {
                System.out.println("I" + j + " -> H" + i + ": " + hiddenweights[i][j]);
            }
        }
        for (int i = 0; i < outsize; i++) {
            System.out.println("Output neuron " + i);
            for (int j = 0; j < hiddensize; j++) {
                System.out.println("H" + j + " -> O" + i + ": " + outweights[i][j]);
            }
        }
