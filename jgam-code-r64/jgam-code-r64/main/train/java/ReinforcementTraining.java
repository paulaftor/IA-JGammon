...
public class ReinforcementTraining extends Thread implements ConsoleApp {
    ...
    SigmoidSimpleNeuralNet normalneuralnet;

    SigmoidSimpleNeuralNet raceneuralnet;
    ...
    public ReinforcementTraining() throws Exception {
        ...
        randomAi.init();
        ai = new NeuralCombiAI();

        // prevent from reading a net from resources.
        ai.setAIForCategory(CombiAI.Mode.NORMAL, new NormalNeuralNetAI());
        ai.setAIForCategory(CombiAI.Mode.RACE, new ProbRaceNeuralNetAI());
        ai.init();
    }
            ...
            while (true) {
                ...
                while (setup.winstatus() == 0) {
                    ...
                    if (random.nextDouble() < exploration) {
                        ...
                        debugout(3, "Category: "
                                + CombiAI
                                        .getCategory(setup, setup
                                                .getPlayerAtMove()));
                        debugout(3, "Win Chance: " + ai.probabilityToWin(setup));
                        ...
                        setup.switchPlayers();
                        double otherswin = ai.probabilityToWin(setup);
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (args.length > 0) {
            ...
            ConsoleFrame.commandline(t, "Reinforcement JGAMMON Trainer NORMAL AI");
        }
    }
    ...
    public void newNet(double range, double range2) {
        ...
        SigmoidSimpleNeuralNet racenet = new SigmoidSimpleNeuralNet(
    ...
    public void setNormalNet(SigmoidSimpleNeuralNet neuralNetwork) {
        normalneuralnet = neuralNetwork;
        ai.setAIForCategory(CombiAI.Mode.NORMAL, new NormalNeuralNetAI(normalneuralnet));
    }

    public void setRaceNet(SigmoidSimpleNeuralNet neuralNetwork) {
        raceneuralnet = neuralNetwork;
        ai.setAIForCategory(CombiAI.Mode.RACE, new ProbRaceNeuralNetAI(raceneuralnet));
    }
    ...
    public boolean command(String command) throws Exception {
        ...
        if (args[0].equals("loadnormalnet")) {
            ObjectInputStream ois = new ObjectInputStream(new FileInputStream(
                    args[1]));
            setNormalNet((SigmoidSimpleNeuralNet) ois.readObject());
            ois.close();
        } else
        ...
        if (args[0].equals("loadracenet")) {
            ObjectInputStream ois = new ObjectInputStream(new FileInputStream(
                    args[1]));
            setRaceNet((SigmoidSimpleNeuralNet) ois.readObject());
            ois.close();
        } else
