...
public class Pip2TurnsData {
    ...
    static CombiAI ai = new NeuralCombiAI();
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < 400; i++) {
            ...
            int count = 0;
            // mach noch mal 10 Z�ge
            while (!inst.winstatus() && count < 15) {
                inst.roll();
                MoveChain m = ai.makeMoves(inst);
                inst.performMoves(m);
                count++;
            }
...
//            inst.debugOut();
    ...
    static class TrainingSetup extends ArrayBoardSetup {
        ...
        void performMoves(MoveChain chain) {
            for (SingleMove m : chain) {
                if (m.player() == 1) {
                    checkers1[m.from()]--;
                    checkers1[m.to()]++;
                } else {
